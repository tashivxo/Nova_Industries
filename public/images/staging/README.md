# Portfolio source assets (staging)

Source files for Tashiv to wire into Nova Industries later.

**Not wired into `projects.ts` yet — Tashiv will choose placement.**

Do not treat this folder as live case-study media. Template images under `public/images/projects/` are unchanged.

## Inventory

### `crosswith/`

Crosswith / SWAG campaign and identity stills.

- **Source:** [tashivxo/crosswithswag](https://github.com/tashivxo/crosswithswag) — Marcus winter-drop artboards plus stitch compositions (hero landing + Kloof Street edition layout).
- `crosswith-01.jpg` … `crosswith-08.jpg` — winter-drop campaign stills (Kloof Street Nights posters and garment shots).
- `crosswith-hero-landing.png` — stitch/site hero landing.
- `crosswith-kloofstreet.png` — stitch Kloof Street edition layout.
- `swag-logo.svg` — SWAG wordmark (white stroke, dark-friendly).

### `freeperiod/`

- **Source:** [tashivxo/freePeriod](https://github.com/tashivxo/freePeriod) pictogram.
- `freeperiod-pictogram.png`
- `freeperiod-pictogram-colour.png`
- The two pictogram files currently hash-identical (colour mark as uploaded).
- Free Period UI screenshots may arrive in a follow-up. A live capture from https://free-period.vercel.app was skipped: headless Chrome hit the site’s client error fallback (“This page couldn’t load”) rather than a clean marketing view, so nothing from that attempt is staged.

### `birdseye/`

- Birds Eye / BXRDS logo **still needed** — no files copied (do not invent).
- Tashiv confirmed Bakang assets live in [odwamanitshana/bxrds-eye-website](https://github.com/odwamanitshana/bxrds-eye-website). This agent **could not list** logos/marks there: GitHub App installation and environment `repos` only include `tashivxo/Nova_Industries`, so the private repo 404s on view/clone/API.
- **Nova Industries check:** `public/images/` has `blog/`, `hero/`, `projects/` (template stills), `ui/`, and `staging/` only. No existing `birdseye/` or `bxrds/` media folders besides this placeholder (`birdseye/README.md`).
- See `birdseye/README.md` for the access log and how to finish.
